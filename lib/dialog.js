...
var Dialog = Node.extend({
	...
	show:function(content, options){
		...
		this.animateClass = options.animate;
	...
	hide:function(options){
		var self = this;
		options = options || {};
		var animateClass = this.animateClass || options.animate;
		...
		if(animateClass){
			if(options.animate){
				...
				this.$elem.removeClass(this.animateClass);
			}
			setTimeout(function(){
				if(options.animateEnd){
					var result = options.animateEnd();
					if(result !== false){
						self.$elem.hide();
						self.$mask.hide();
						if(options.animate){
							self.$elem.removeClass(options.animate);
							self.$elem.removeClass(self.animateClass);
						}
					}
				}
				else{
