	...
	var stats = {
		...
		fixedData : [
		 ...
		 _send:function(params){
		 	var img = new Image();
		 	img.src = this.requestUrl+'?'+params.concat(this.fixedData).join('&');
		 },
		 ...
		 pv:function(domReadyTime, loadEventTime){
		 	...
		 	setTimeout(function(){
		 		...
		 		params = [
		 			'et=87',
		 			'ep={"netAll":1,"netDns":0,"netTcp":0,"srv":39,"dom":'+(domReadTime?domReadTime:0)+',"loadEvent":'+(loadEventTime?loadEventTime:0)+',"qid":"","bdDom":0,"bdRun":0,"bdDef":0}',
		 			'rnd='+Math.round(Math.random() * 2147483647),
		 			'tt='+encodeURIComponent(document.title)
		 		];
		 		self._send(params);
